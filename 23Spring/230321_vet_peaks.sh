...
## Example command: sbatch 230321_vet_peaks.sh $1
    # $1 = protein name
...
then
    ...
    # find peaks with >2igg in >=2 replicates
    ...
    # count peaks with >2igg in >=2 replicates
...
then
    ...
    # find peaks with >4igg in >=2 replicates
    ...
    # count peaks with >4igg in >=2 replicates
...
### 6. correlation between reads from individual replicates in >2 igg peaks (in >= 2 replicates)
...
### 7. correlation between reads from individual replicates in >4 igg peaks (in >= 2 replicates)
...
### 8. correlation between reads from individual replicates in all >2 igg peaks
...
then
    ...
    do
        ...
        do
            ...
            ## correlation between ripi_rpm and ripj_rpm
            # get 2 lists
            cat $1_fc_rpm_2igg.txt | sed 1d | cut -f${ripi_col} > $1_ripi_rpm.txt
            cat $1_fc_rpm_2igg.txt | sed 1d | cut -f${ripj_col} > $1_ripj_rpm.txt
            # corr in python:
            pearson_r=$(python /proj/calabrlb/users/Zhiyue/23_sp/code/230313_corr.py $1_ripi_rpm.txt $1_ripj_rpm.txt)
...
### 9. correlation between reads from individual replicates in all >4 igg peaks
...
then
    ...
    cat $1_fc_rpm.txt | awk -v c1=$rip_all_col -v c2=$igg_col  '{if(NR == 1) print $0; else if($c1 > 4 * $c2) print $0}' > $1_fc_rpm_4igg.txt
    ...
    do
        ...
        do
            ...
            ## correlation between ripi_rpm and ripj_rpm
            # get 2 lists
            cat $1_fc_rpm_4igg.txt | sed 1d | cut -f${ripi_col} > $1_ripi_rpm.txt
            cat $1_fc_rpm_4igg.txt | sed 1d | cut -f${ripj_col} > $1_ripj_rpm.txt
            # corr in python:
            pearson_r=$(python /proj/calabrlb/users/Zhiyue/23_sp/code/230313_corr.py $1_ripi_rpm.txt $1_ripj_rpm.txt)
